...
import {
  ...
  Search, Download, BarChart3, PieChart, Activity, Target, CheckCircle
} from "lucide-react";
import Chart from "chart.js/auto";
import "../../styles/Ratinganalytics.css";

export default function ServiceAnalytics({ services = [], bookings = [], onBack }) {
  ...
  // FIXED EMPTY STATE - ALL TAGS CLOSED
    ...
    >
      ...
      <div className="charts-grid">
        <div className="chart-card wide">
          ...
          <div className="chart-wrap"><canvas id="revenue-service-chart" /></div>
        </div>
        ...
        <div className="chart-card">
          ...
          <div className="chart-wrap"><canvas id="rating-radar-chart" /></div>
        </div>
        ...
        <div className="chart-card">
          ...
          <div className="chart-wrap"><canvas id="popularity-polar" /></div>
        </div>
        ...
        <div className="chart-card wide">
          ...
          <div className="chart-wrap"><canvas id="revenue-trend-chart" /></div>
        </div>
        ...
        <div className="chart-card">
          ...
          <div className="chart-wrap"><canvas id="bookings-doughnut" /></div>
        </div>
        ...
        <div className="chart-card">
          ...
          <div className="chart-wrap"><canvas id="growth-area" /></div>
        </div>
      </div>
      ...
      <div className="insights-section">
        ...
        <div className="insight-cards">
          <motion.div className="insight-card success" whileHover={{ y: -5 }}>
            <TrendingUp size={24} />
            <div>
              <h4>Best Performing Service</h4>
              <p><strong>{topService?.name || 'N/A'}</strong> generates ₹{(topService?.revenue || 0).toLocaleString()} with {topService?.bookings || 0} bookings</p>
              <span className="insight-action">Boost promotion →</span>
            </div>
          </motion.div>

          <motion.div className="insight-card warning" whileHover={{ y: -5 }}>
            ...
            <div>
              ...
              <span className="insight-action">View strategy →</span>
            </div>
          </motion.div>

          <motion.div className="insight-card info" whileHover={{ y: -5 }}>
            <Star size={24} />
            <div>
              <h4>Highest Rated</h4>
              <p><strong>{highestRated?.name || 'N/A'}</strong> leads with {highestRated?.rating || 0}★ rating. Leverage for testimonials.</p>
              <span className="insight-action">Create campaign →</span>
            </div>
          </motion.div>

          <motion.div className="insight-card danger" whileHover={{ y: -5 }}>
            <AlertTriangle size={24} />
            <div>
              <h4>Low Booking Alert</h4>
              <p>{filteredServices.filter(s => s.bookings < 5).length} services have less than 5 bookings this month</p>
              <span className="insight-action">Optimize listings →</span>
            </div>
          </motion.div>
        </div>
      </div>
            ...
            <tbody>
              {filteredServices.map((s, i) => (
                ...
                >
                  ...
                  <td>{s.bookings || 0}</td>
                  <td className="revenue-cell">₹{(s.revenue || 0).toLocaleString()}</td>
                  <td>
                  ...
                  <td className="profit-cell">₹{(s.profit || 0).toLocaleString()}</td>
                  <td>
